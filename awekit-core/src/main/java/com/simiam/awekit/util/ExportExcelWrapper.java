...
public class ExportExcelWrapper<T> extends ExportExcelUtil<T> {
    ...
    public void exportExcelByFields(String fileName, String title, String[] headers, String[] fileds, Collection<T> dataset, HttpServletResponse response, String version) {
        try {
            response.setContentType("application/vnd.ms-excel");
            response.addHeader("Content-Disposition", "attachment;filename=" + URLEncoder.encode(fileName, "UTF-8") + ".xls");
            if (StringUtils.isBlank(version) || EXCEL_FILE_2003.equals(version.trim())) {
